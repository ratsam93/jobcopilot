...
def list_oss_integrations(active_settings: Settings = settings) -> OSSIntegrationRegistry:
    ...
    integrations = [
        ...
        OSSIntegrationStatus(
            ...
            production_status="not_on_request_path",
            direct_usage="Fit scoring is currently custom; CV-Matcher can be wrapped as an optional scoring comparison adapter.",
            local_clone_present=_clone_exists("CV-Matcher"),
            config_keys=["JOBCOPILOT_CV_MATCHER_ENABLED"],
            safety_notes=["Do not replace final scoring blindly; use as a scored reference signal."],
            next_action="Wrap keyword extraction/scoring as an explainable secondary scorer.",
        ),
        OSSIntegrationStatus(
            key="reactive-resume",
            ...
            production_status="not_started_in_main_compose",
            direct_usage="Not on production request path; templates/editor concepts can become a resume-rendering service.",
            ...
            next_action="Expose selected templates through a small internal rendering adapter.",
        ),
        OSSIntegrationStatus(
            key="open-resume",
            name="OpenResume",
